...
class Aggregator(UnfittingTransformer, DataFrameTransformer):
    ...
    def _fit_transform(self, data_pod: DataPod) -> DataPod:  # type: ignore[override]
        ...
        source_df: pd.DataFrame = source_df.groupby(self.groupby_col_names).agg(self.agg_functions)
        ...
        source_df.columns = self.target_col_names

        target_df = pd.merge(target_df, source_df, on=self.groupby_col_names, how="left")
